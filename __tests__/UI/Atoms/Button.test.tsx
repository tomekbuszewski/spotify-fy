...
import {
  ...
  WithIcon,
  CallToAction,
} from "@ui/Atoms/Button/Button.stories";
...
describe("Atoms/Button tests", () => {
  it("should render properly with default props", async () => {
    ...
    expect(getByTestId(TEST_IDS.NORMAL)).toBeInTheDocument();
    expect(getByTestId(TEST_IDS.NORMAL)).toHaveTextContent(TEXT);
  });

  it("should render properly with icon", () => {
    const { container, getByTestId } = renderWithDeps(<WithIcon />);

    expect(getByTestId(TEST_IDS.WITH_ICON)).toBeInTheDocument();
  ...
  it("should render properly as call to action", () => {
    ...
    expect(getByTestId(TEST_IDS.CTA)).toBeInTheDocument();
    expect(getByTestId(TEST_IDS.CTA)).toHaveTextContent(TEXT);
  });
});
